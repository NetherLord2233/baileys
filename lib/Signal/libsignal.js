...
import { signal } from "libsignal/src/protobufs.js";
...
const PreKeyWhisperMessage = signal
...
function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
  ...
  const migratedSessionCache = new LRUCache({ ttl: 3 * 24 * 60 * 60 * 1e3, ttlAutopurge: true, updateAgeOnGet: true });
  ...
  const repository = { decryptGroupMessage({ group, authorJid, msg }) {
  ...
  }, lidMapping, async validateSession(jid) {
    try {
      const addr = jidToSignalProtocolAddress(jid);
    ...
    return parsedKeys.transaction(async () => {
      const migrationOps = deviceJids.map((jid) => {
        ...
        return { fromJid: jid, toJid: lidWithDevice, pnUser: fromDecoded.user, lidUser: toDecoded.user, deviceId: fromDecoded.device || 0, fromAddr: jidToSignalProtocolAddress(jid), toAddr: jidToSignalProtocolAddress(lidWithDevice) };
      });
...
function signalStorage({ creds, keys }, lidMapping) {
  ...
  }, removePreKey: (id) => keys.set({ "pre-key": { [id]: null } }), loadSignedPreKey: () => {
  ...
  }, getOurRegistrationId: () => creds.registrationId, getOurIdentity: () => {
